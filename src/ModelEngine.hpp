...
public:
    ...
    int infer(const std::string& model_name, const std::string& prompt, std::string& output);

private:
    std::map<std::string, DenseModel>& local_models;
    SQLiteRouter& sqlite_router;

    int infer_cloud(const std::string& model_name, const std::string& provider_url, const std::string& api_key, const std::string& prompt, std::string& output);
